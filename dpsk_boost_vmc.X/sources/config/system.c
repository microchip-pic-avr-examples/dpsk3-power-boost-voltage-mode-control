...
volatile uint16_t sysUserPeriperhals_Initialize(void) {
    ...
    // Initialize boost converter current sense op-amp
    retval &= sysOpAmp_Initialize(BOOST_ISNS_OPA_INSTANCE, false); // Initialize op-amp #2 used to amplify the low-side current sense of the boost converter
    
    // Initialize DAC buffer op-amp
    retval &= sysOpAmp_Initialize(DAC_BUFFER_OPA_INSTANCE, true); // Initialize op-amp #3 used to drive the reference voltage for current sense amplifiers
...
volatile uint16_t sysUserTasks_Initialize(void) {
    ...
    retval &= appPowerSupply_Initialize(); // Initialize BOOST converter object and state machine
