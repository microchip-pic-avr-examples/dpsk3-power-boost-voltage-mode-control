...
- [Boost Converter State Machine](#sfw_2)
  ...
  - [VCap Charge-Up Delay (VCAP)](#sfw_2e)
...
The main purpose of the Power-On Delay is to account for input power bouncing, which may occur when a cable has to be plugged in to attach power to the input, or to wait until the input capacitance has fully charged up and stable input power is available.

[[back](#startDoc)]

<span id="sfw_2e"></span>
### d) VCap Charge-Up Delay (VCAP)
This delay state is a specific state for boost converters, where the power input is directly connected to the output of the power supply through the high-side rectifier diode resp. the body diode of the high-side FET in a synchronous topology. Users can specify the minimum time the state machine should wait until the output voltage has reached a charging level `VOUT = VIN - VDROP`, where `VDROP` specifies the voltage drop across the rectifier diode. Users can also specify a timeout period after which the state machine will reset should the output voltage not reach the expected voltage range within the given timeout period.

This wait state can be disabled by setting the expected charge-up time to zero (=0).
...
The setting for the nominal output voltage is defined like shown below:
