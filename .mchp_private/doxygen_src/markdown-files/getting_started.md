...
- [Boost Converter State Machine](#sfw_2)
  ...
  - [Reset](#sfw_2b)
  - [Standby](#sfw_2c)
  - [Output Capacitor Charge-Up Monitor](#sfw_2d)
  - [Power-On Delay (POD)](#sfw_2e)
  - [Launch Voltage Ramp](#sfw_2f)
  - [Voltage Ramp-Up](#sfw_2g)
...
<span id="sfw_2d"></span>
#### d) Output Capacitor Charge-Up Monitor (CHRG)
The boost converter output capacitor will charge up automatically as soon as input power is applied. This charge-up period may vary depending on the the size of the output capacitance and components used. Hence, this additional wait-state has been introduced in which the controller monitors the output voltage waiting for it to settle before the boost converter is started up. This additional wait state helps to reduce component stress when driving high capacitive loads. Users can define the waiting period in which the output voltage should remain stable before the converter is starting up, as well as the expected voltage drop between input and output voltage (usually determined by the no-load rectifier diode forward voltage). This state also counts up a timeout period, which will reset the state machine initiating a restart attempt if the output voltage does not settle as expected.

This state can be skipped by setting the wait period to zero (= 0).

[[back](#startDoc)]

<span id="sfw_2e"></span>
#### e) Power-On Delay (POD)
Once the boost converter has been cleared the state machine will execute the startup procedure starting with the Power On Delay. This is just a simple delay during which the converter will remain inactive but the fault handler will observe the values generated by the ADC for occurring fault conditions.

[[back](#startDoc)]

<span id="sfw_2f"></span>
#### f) Launch Voltage Ramp
...
The setting for the nominal output voltage is entered by changing the given defines below:

    #define BOOST_VOUT_NOMINAL           (float)15.000  // Nominal output voltage in [V]
    #define BOOST_VOUT_TOLERANCE_MAX     (float) 0.500  // Output voltage tolerance [+/-]
    #define BOOST_VOUT_TOLERANCE_MIN     (float) 0.200  // Output voltage tolerance [+/-]
